...
- Authorize by `POST /v1/data/agentcontrol/authz/result` against OPA with `{"input": …}`, mapping the response `result` object plus `decision_id` into a `ControlResult`. (`refs/opa/v1/server/types/types.go:242-282`.) **The trailing `/result` is load-bearing** — querying the bare package path returns every public rule/var in the package as siblings, not the decision object; caught only by running a real `opa run --server`, not by the mocked test transport (research R10).
